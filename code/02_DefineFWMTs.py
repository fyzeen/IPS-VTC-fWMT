        ...
        for hemi in hemis:
            ...
            for tck_type in tck_types:
                ...
                for thresh in ["t>0", "t>2", "t>3"]:
                    ...
                    print("Defined fWMT for 'all_rois_nodes'")

                    whole_kastner_rois = op.join(
                        rois_base, hemi+".Kastner2015.subsetted.projected.gmwmi_intersected.binarized.withVTC.nii.gz")
                    whole_kastner_rois_out = op.join(out_base, "VTCtoWholeIPS", thresh, tck_type)
                    whole_kastner_nodes = available_flocfaces_rois[1] + [18]
                    whole_kastner_nodes = ", ".join(str(int(node)) for node in whole_kastner_nodes)

                    intersect_tck_with_rois(tck, whole_kastner_rois, whole_kastner_rois_out, whole_kastner_nodes)
                    print("Defined fWMT for 'whole_kastner_nodes'")
                    ...
                    whole_vtc_nodes = ", ".join(str(int(node)) for node in whole_vtc_nodes)
                    

                    intersect_tck_with_rois(tck, whole_vtc_rois, whole_vtc_rois_out, whole_vtc_nodes)
                    print("Defined fWMT for 'whole_vtc_nodes'")
